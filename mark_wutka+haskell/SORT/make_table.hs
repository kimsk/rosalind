...
computeReversalsTable reversalTable depth =
    ...
    where
        ...
        compareReversalDepth depth (k,v) = depth == (fst v)
        tryReversals reversalTable (reversal, (revDepth,revList)) =
            foldl' tryNewReversal reversalTable [(doReversal reversal s l,s+1,l+s,revList) | l <- [2..10], s <- [0..(10-l)]]
        tryNewReversal reversalTable (attemptedReversal,start,length,revList) =
            if not $ Map.member attemptedReversal reversalTable then
                Map.insert attemptedReversal (depth+1,[start,length]:revList) reversalTable
            else
                reversalTable
...
makeTableLines reversalTable =
    ...
    where
        makeTableLine (reversal, (depth,revList)) = reversal ++ " " ++ (show (reverse revList))
