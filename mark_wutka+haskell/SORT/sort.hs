...
-- permutations of 01234567890 and the reversals it takes to get from
-- 0123456789 to that permutation.
--
...
makeReversalTable tableLines =
    ...
    where
        makeTableEntries :: [String] -> (String,[[Int]])
        makeTableEntries [perm,reversals] = (perm,read reversals)

findReversal :: Map.Map String [[Int]] -> [String] -> [[Int]])
findReversal reversalTable [fromStr, toStr] =
    (Map.findWithDefault [[-1,-1]] computeTarget reversalTable)
    where
        from = words fromStr
        to = words toStr
...
main = do
  ...
  let reversals = findReversal reversalTable $ take 2 (lines fileLines)
  putStrLn . show $ length reversals
  putStr . unlines $ map (\r -> unwords $ map show r) reversals
